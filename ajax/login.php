...
echo success('复制下面的代码，放在密码找回页.<br>代码: <b>'.$code.'</b>');
...
if ($countCode > 0)
{
	...
	while($show = $SQLSelectAPI ->fetch())
	{
		...
		echo success('您的密码: <strong>'.$passwordReq.'</strong>');
	}
...
die(error('太多次失败的尝试.请等待 '.$date.' 秒后重试.'));
}
...
die(error('你是被禁止的.原因: '.htmlspecialchars($ban)));
}
...
echo success(' 登录成功.重定向...<meta http-equiv="refresh" content="3;URL=index.php">');
}
...
{
die(error(' Email 地址无效'));
}
//Compare first to second password
if ($password != $rpassword)
{
die(error(' 两次输入的密码不一致'));
}
//Make registeration
$SQL = $odb -> prepare("SELECT * FROM `users` WHERE `username` = :username");
$SQL -> execute(array(':username' => $username));
...
$_SESSION['username'] = $userInfo['username'];
$_SESSION['ID'] = $userInfo['ID'];
echo success(' 您已成功注册！正在跳转登录页...<meta http-equiv="refresh" content="3;url=index.php">');
}
?>